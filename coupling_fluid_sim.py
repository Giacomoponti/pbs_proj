...
u_valid = ti.Vector.field(n=2, dtype=float, shape=shape)
...
# marker particles
particles = ti.Vector.field(n=2, dtype=float, shape=(N))
particle_radius = dx / 1.414 # sqrt(2)

positives = ti.field(dtype=float, shape=(4))
...
def local_signed_distance_function_to_box(pos_local):
    ...
    i = 0
    if distance_left >= 0:
        positives[i] = distance_left
        i += 1
    if distance_right >= 0:
        positives[i] = distance_right
        i += 1
    if distance_bottom >= 0:
        positives[i] = distance_bottom
        i += 1
    if distance_top >= 0:
        positives[i] = distance_top
        i += 1

    return_val = 0.0
    if i == 2:
        return_val = ti.math.sqrt(positives[0]*positives[0] + positives[1]*positives[1])
    elif i == 1:
        return_val = positives[0]
    else:
...
def update_rigid_body_fields():
    ...
    for i, j in nodal_rigid_phi:
        ...
        nodal_rigid_phi = get_signed_distance_function_to_box(location)
    ...
    for i, j, k, m in ti.ndrange(window_width, window_height, window_width, window_height):
        ...
        if center_phi < 0:
            ...
            if other_phi < 0:
                ...
                if abs(val) > 0.000001:
                    K[i + window_width*j, k + window_width*m] += val


@ti.kernel
def fill_matrix_again_to_solve(K: ti.types.sparse_matrix_builder(), F_b: ti.types.ndarray()):
    for i in ti.ndrange(N):
        if nonzeros_count_for_rows[i] == 0:
            K[i, i] += 1.0 # need to set plus 1 here because the = operator is not defined for triplets
            F_b[i] = 0
@ti.kernel
...
def main():
    ...
    for frame_counter in range(200):
        ...
        window.set_image(supersize_color_field)
        ...
        img = supersize_color_field.to_numpy()
