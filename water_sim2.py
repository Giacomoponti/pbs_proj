...
def reset_ball_particles():
    ...
    counter = 0
    for n, m in px:
        if n >= 2*nx - reserved_particles and m >= 2*nx - reserved_particles:
            average_velocity += pv[n, m]
            average_position += px[n, m]
            counter += 1
    average_position /= counter
    average_velocity /= counter
    for n, m in px:
        if n >= 2*nx - reserved_particles and m >= 2*nx - reserved_particles:
            pv[n, m] = average_velocity
            #initialize the particle position
            i = 0.5 + n - (2*nx - reserved_particles) - reserved_particles//2
            j = 0.5 + m - (2*ny - reserved_particles) - reserved_particles//2
            px[n, m] = average_position + ti.Vector([i, j])

@ti.kernel
def apply_bc_to_particles():
    for n, m in px:
        if 1 == pf[n, m]:
            if px[n, m].x < 0.0:
                px[n, m].x = 0.0
...
def init_particles():
    ...
    for n, m in px:
        if n >= 2*nx - reserved_particles and m >= 2*nx - reserved_particles:
            pv[n, m] = ti.Vector([0.0, 0])
            #initialize the particle position
            i = 2*nx - n - reserved_particles//2
            j = 2*ny - m - reserved_particles//2
            px[n, m] = ti.Vector([nx // 2 + i, ny // 2 + j])

            #update the marker
            marker[nx // 2 + i, ny // 2 + j] = FLUID
...
class Viewer:
    ...
    def draw(self, gui):
        ...
        #             np.array([[1.0 / nx, 1.0 / nx]]), radius=1.0, color=0xFFFFFF)
...
def shoot_water(offset: int, angle: float):
    ...
    vel_vector = rotation @ ti.Vector([0, 200])
    ...
    for j in range (20):
        ...
        #update the particle velocity 
            pv[offset + nx // 2 + i, j] = vel_vector
            pv[offset + nx // 2 - i, j] = vel_vector
            #initialize the particle position
            px[offset + nx // 2 + i, j] = ti.Vector([offset + nx // 2 + i, j])
            px[offset + nx // 2 - i, j] = ti.Vector([offset + nx // 2 - i, j])

            #update the marker
            marker[offset + nx // 2 + i, j] = FLUID
            marker[offset + nx // 2 - i, j] = FLUID

            #update the particle flag
            pf[offset + nx // 2 + i, j] = 1
            pf[offset + nx // 2 - i, j] = 1

            #update the pressure
            pressure[offset + nx // 2 + i, j] = 0
            pressure[offset + nx // 2 - i, j] = 0

            #update the divergence
            divergence[offset + nx // 2 + i, j] = 0
            divergence[offset + nx // 2 - i, j] = 0

            #update the velocity
            ux[offset + nx // 2 + i, j] = 0
            ux[offset + nx // 2 - i, j] = 0

            uy[offset + nx // 2 + i, j] = 0
            uy[offset + nx // 2 - i, j] = 0
    ...
    #         pv[n, m] = ti.Vector([0.0, 200])
    #         #initialize the particle position
    #         px[n, m] = ti.Vector([nx // 2 + i, j])
    ...
    # if next_particle_insert_counter[None] + shoot_count + reserved_particles >= 2 * nx:
    #     next_particle_insert_counter[None] = 0   
